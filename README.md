...
- AI-объяснения до **280 символов**: проверенная дата и одна-две цитаты из профиля. API возвращает идентификаторы подтверждающих фактов — `evidence_ids`.
...
Модель задаётся через `OPENAI_MODEL`. Текущая версия AI-модуля: `ranking-v6:evidence-v3:fallback-v2`. Зависимости закреплены в [requirements.txt](requirements.txt) и [requirements-ai.txt](requirements-ai.txt).
...
На версии `ranking-v6:evidence-v3:fallback-v2` 23.09.2026:
