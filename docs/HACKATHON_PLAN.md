...
- Одним запросом попросить сравнить всех кандидатов и вернуть `selected`: упорядоченный массив объектов с `id`, `reason` (1–2 предложения) и `evidence_ids`.
...
- Имена, цены, город и прочие поля карточки брать из БД, а не из текста модели. Объяснения должны ссылаться на проверенные условия и факты; отсутствие доказательства желаемого свойства обозначается словами.
...
**Ограничение времени:** один AI-запрос на всех прошедших фильтры кандидатов, общий таймаут около 5 секунд, без длинных повторных попыток. Для пустого результата AI вообще не нужен. Задержку измеряем на реальных запросах уже в первый час.
...
| --- | --- | --- |
| **1 — Backend / Integration** | API, БД, строгие фильтры, проверка и сохранение решений, Docker, финальная интеграция | Backend, схемы API, зависимости, Compose |
| **2 — AI / Data** | Разбор CSV, критерии AI-ранжирования, выбор TOP-3, объяснения, fallback | Импорт, AI-модуль, тестовые данные и проверки выбора |
...
| 2 | Разобрать CSV; передать AI более трёх реальных допустимых кандидатов и контекст, получить TOP-3 с фактами; подготовить fallback | Работающий прототип AI-выбора и объяснений, замер задержки |
...
- [ ] AI действительно выбирает TOP-3 из всех прошедших фильтры; проверен запрос с более чем тремя допустимыми кандидатами.
- [ ] Пожелания передаются AI и входят в ключ сохранённого решения.
- [ ] Неизвестные/повторные ID и неверное число рекомендаций отклоняются.
