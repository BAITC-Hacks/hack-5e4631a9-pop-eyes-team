...
- Следующий общий checkpoint: включить `AI_MODULE=ai_module`, задать актуальную `AI_VERSION`, применить `003_recommendations.sql` к существующей БД и проверить полный сценарий. Участник 2 проверяет фактических кандидатов, порядок и ссылки; участник 3 — форму и метки режима. Локальные замеры AI не заменяют проверку с БД, повтором запроса и перезапуском.
...
- Одним запросом попросить сравнить всех кандидатов и выбрать ID, ссылки на факты и фрагмент пожеланий. AI-модуль собирает `reason` из точных цитат и возвращает согласованный `selected` с `id`, `reason`, `evidence_ids`; backend не зависит от внутреннего формата модели.
